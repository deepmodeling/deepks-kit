...
def t_flat_pdms_parity_full(t_dm: torch.Tensor, t_proj: torch.Tensor,
                            basis_info: BasisInfo, cg_coeffs: ClebschGordan) -> torch.Tensor:

    t_pdm_ori = t_make_pdm(t_dm, t_proj)
    t_pdm_ori = 0.5 * (t_pdm_ori + torch.swapaxes(t_pdm_ori, -1, -2))
    ...
    desc = desc_gen(t_pdm_ori)
    desc_t = desc_gen(torch.swapaxes(t_pdm_ori, -1, -2))
...
def t_flat_pdms_parity_tiru(t_dm: torch.Tensor, t_proj: torch.Tensor,
    ...
    for idx1, l1 in enumerate(ls):
        ...
        for idx2, l2 in enumerate(ls[:idx1+1]):
            nl2 = nls[idx2]
            parity = int((-1)**(l1+l2) < 0)  # even -> 0, odd -> 1
            t_mat = t_pdm[..., mat_idx[idx1]:mat_idx[idx1+1], mat_idx[idx2]:mat_idx[idx2+1]]
            t_mat = t_mat.reshape((*t_mat.shape[:-2], nl1, 2*l1+1, nl2, 2*l2+1))
            for l3 in range(abs(l1-l2), l1+l2+1):
                cg = cg_coeffs(l1, l2, l3)
                ...
                if l1 == l2:
                    if l3 % 2 == 0:
                        indices = torch.triu_indices(nl1, nl1)
                    else:
                        indices = torch.triu_indices(nl1, nl1, 1)
                    t_flat_mat = t_trans_mat[..., indices[0], indices[1], :].reshape(
                        (*t_trans_mat.shape[:-3], -1, t_trans_mat.shape[-1]))
                    t_tmp_l_list[l3_idx].append(t_flat_mat)
                else:
                    t_tmp_l_list[l3_idx].append(t_trans_mat.reshape((*t_trans_mat.shape[:-3], nl1*nl2, 2*l3+1)))
...
def t_get_corr(model, t_dm, t_proj, basis_info, cg_coeffs, with_vc=True):
    ...
    if torch.linalg.norm(vc - torch.swapaxes(vc, -1, -2)) > 1e-8:
        raise RuntimeError("vc not symmetric")
...
class NetMixin(CorrMixin):
    ...
    def __init__(self, model, proj_basis=None, triu=True, device=DEVICE):
        ...
        self.basis_info = BasisInfo(self._pbas, triu=triu)
...
class DSCF(NetMixin, PenaltyMixin, dft.rks.RKS):
    ...
    def __init__(self, mol, model, xc="HF", proj_basis=None, penalties=None, triu=False, device=DEVICE):
        # base method must be initialized first
        dft.rks.RKS.__init__(self, mol, xc=xc)
        # correction mixin initialization
        NetMixin.__init__(self, model, proj_basis=proj_basis, triu=triu, device=device)
