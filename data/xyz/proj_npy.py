...
def load_data(dir_name):
    ...
    ec_ij = np.loadtxt(os.path.join(dir_name, 'ec_ij.raw')).reshape(-1, nocc, nocc)
...
def dump_data(dir_name, meta, ehf, emp2, ec_ij, e_data, c_data) :
    ...
    assert(all(ec_ij.shape == np.array([nframe, nocc, nocc], dtype = int)))
    np.save(os.path.join(dir_name, 'e_hf.npy'), ehf) 
    np.save(os.path.join(dir_name, 'e_mp2.npy'), emp2) 
    np.save(os.path.join(dir_name, 'ec_ij.npy'), ec_ij)
...
def main():
    ...
    for xf, md in zip(args.xyz_file, args.mo_dir):
        meta, e_hf, e_mp2, ec_ij, e_data, c_data = proj_frame(xf, md, 
    ...
    all_ec_ij = np.concatenate(all_ec_ij)
    ...
    dump_data(args.dump_dir, meta, all_e_hf, all_e_mp2, all_ec_ij, (all_e_occ, all_e_vir), (all_c_occ, all_c_vir))
    print("done")
